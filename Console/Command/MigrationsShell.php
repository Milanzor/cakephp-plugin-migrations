 ...
 * Migration shell.
 */
class MigrationsShell extends AppShell {
    ...
    public function run() {
        ...
        if (isset($this->args[0]) && in_array($this->args[0], ['up', 'down'])) {
            $once = true;
    ...
    protected function _execute($options, $once) {
        ...
        try {
            ...
            $response = $this->in(__d('migrations', 'Do you want to mark the migration as successful?. [y]es or [a]bort.'), ['y', 'a']);
     ...
     */
    protected function _singleStepOptions($mapping, $latestVersion, $default = []) {
        $options = $default;
    ...
    public function generate() {
        ...
        if ( ! empty($this->args)) {
            ...
            if ($oldSchema !== false) {
                ...
                if ($response === false) {
                    $response = $this->in(__d('migrations', 'Do you want to compare the schema.php file to the database?'), ['y', 'n'], 'y');
                }
                ...
                if (strtolower($response) === 'y') {
                    ...
                    $response = $this->in(__d('migrations', 'Do you want to compare the database to the schema.php file?'), ['y', 'n'], 'y');
                    if (strtolower($response) === 'y') {
                ...
                $response = $this->in(__d('migrations', 'Do you want to generate a dump from the current database?'), ['y', 'n'], 'y');
                if (strtolower($response) === 'y') {
        ...
        if ($fromSchema && isset($comparison)) {
            $response = $this->in(__d('migrations', 'Do you want to update the schema.php file?'), ['y', 'n'], 'y');
            if (strtolower($response) === 'y') {
                $this->_updateSchema();
            }
        }
    ...
    protected function _finalizeGeneratedMigration(&$migration, &$migrationName, &$fromSchema) {
        if ( ! empty($this->params['preview'])) {
            ...
            $preview = $this->in(__d('migrations', 'Do you want to preview the file before generation?'), ['y', 'n'], 'y');
        }
        ...
        foreach ($comparison as $table => $actions) {
            ...
            foreach ($actions as $type => $fields) {
                ...
                if ( ! empty($fields['indexes'])) {
                    $indexes = ['indexes' => $fields['indexes']];
    ...
    protected function _getSchema($type = null) {
        if ($type === null) {
            ...
            return new CakeSchema(['connection' => $this->connection, 'plugin' => $plugin]);
        }
        ...
        $schema = new $name(['connection' => $this->connection, 'plugin' => $plugin]);
        return $schema;
    }
    ...
    protected function _readSchema() {
        $read = $this->Schema->read(['models' => ! $this->params['force']]);
        if ($this->type !== 'migrations') {
    ...
    protected function _generateMigration($name, $class, $migration) {
        ...
        $content = $this->_generateTemplate('migration', ['name' => $name, 'class' => $class, 'migration' => $content]);
