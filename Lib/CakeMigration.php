...
class CakeMigration extends CakeObject {
    ...
    protected function _run() {
        ...
        uksort($this->migration[$this->direction], [$this, 'migration_order']);
    ...
    protected function migration_order($a, $b) {
        $order = ['drop_table', 'rename_table', 'create_table', 'drop_field', 'rename_field', 'alter_field', 'create_field'];
    ...
    protected function _createTable($type, $tables) {
        foreach ($tables as $table => $fields) {
            if ($this->_invokePrecheck('beforeAction', 'create_table', ['table' => $table])) {
                $this->Schema->tables = [$table => $fields];
                $this->_invokeCallbacks('beforeAction', 'create_table', ['table' => $table]);
                try {
            ...
            $this->_invokeCallbacks('afterAction', 'create_table', ['table' => $table]);
        }
        return true;
    }
    ...
    protected function _dropTable($type, $tables) {
        foreach ($tables as $table) {
            ...
            if ($this->_invokePrecheck('beforeAction', 'drop_table', ['table' => $table])) {
                $this->_invokeCallbacks('beforeAction', 'drop_table', ['table' => $table]);
                ...
                $this->_invokeCallbacks('afterAction', 'drop_table', ['table' => $table]);
            }
        }
        return true;
    }
    ...
    protected function _renameTable($type, $tables) {
        foreach ($tables as $oldName => $newName) {
            if ($this->_invokePrecheck('beforeAction', 'rename_table', ['old_name' => $oldName, 'new_name' => $newName])) {
                ...
                $this->_invokeCallbacks('beforeAction', 'rename_table', ['old_name' => $oldName, 'new_name' => $newName]);
                if (@$this->db->execute($sql) === false) {
                    throw new MigrationException($this, __d('migrations', 'SQL Error: %s', $this->db->error));
                }
                $this->_invokeCallbacks('afterAction', 'rename_table', ['old_name' => $oldName, 'new_name' => $newName]);
            }
        }
        return true;
    }
    ...
    protected function _alterTable($type, $tables) {
        foreach ($tables as $table => $fields) {
            ...
            foreach ($fields as $field => $col) {
                $model = new Model(['table' => $table, 'ds' => $this->connection]);
                ...
                if ($type === 'rename') {
                    $data = ['table' => $table, 'old_name' => $field, 'new_name' => $col];
                } else {
                ...
                if ($this->_invokePrecheck('beforeAction', $type . '_field', $data)) {
                    switch ($type) {
                        ...
                        case 'rename':
                            ...
                            $sql = $this->db->alterSchema([
                                $table => ['change' => [$field => array_merge($data, ['name' => $col])]],
                            ]);
                            break;
                    }
    ...
    protected function _alterIndexes($indexes, $type, $table) {
        foreach ($indexes as $key => $index) {
            ...
            if ($this->_invokePrecheck('beforeAction', $type . '_index', ['table' => $table, 'index' => $key])) {
                $this->_invokeCallbacks('beforeAction', $type . '_index', ['table' => $table, 'index' => $key]);
                if (@$this->db->execute($sql) === false) {
                    throw new MigrationException($this, sprintf(__d('migrations', 'SQL Error: %s'), $this->db->error));
                }
            }
            $this->_invokeCallbacks('afterAction', $type . '_index', ['table' => $table, 'index' => $key]);
        }
    }
     ...
     * @throws MigrationException
     */
    protected function _invokeCallbacks($callback, $type, $data = []) {
        if ($this->dry) {
            return true;
        }
     ...
     * @return bool
     */
    protected function _invokePrecheck($callback, $type, $data = []) {
        if ($this->dry) {
            return true;
        }
     ...
     * @return Model
     */
    public function generateModel($name, $table = null, $options = []) {
        if (empty($table)) {
